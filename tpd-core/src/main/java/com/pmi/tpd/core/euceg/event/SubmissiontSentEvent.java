...
/**
 * Raised when a submission sent.
 *
 * @since 3.0
 * @author devacfr
 */
@Audited(converter = SubmissiontSentEvent.AuditConverter.class, priority = Priority.HIGH, channels = {
    EucegChannels.EUCEG, EucegChannels.SUBMISSION })
public class SubmissiontSentEvent extends BaseEvent {
   ...
   *                       the productNumber.
   */
  public SubmissiontSentEvent(@Nonnull final Object source, final long submissionId,
  ...
  public static class AuditConverter implements AuditEntryConverter<SubmissiontSentEvent> {

    @Override
    public IAuditEntry convert(final SubmissiontSentEvent event, final AuditEntryBuilder builder) {
      return builder
