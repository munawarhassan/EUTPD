 ...
 * <p>
 * Designed for direct instantiation but also exposes the static {@link #addApplicationConverters} and
...
public class ApplicationConversionService extends FormattingConversionService {
    ...
    public ApplicationConversionService(final StringValueResolver embeddedValueResolver) {
        this(embeddedValueResolver, false);
    }

    private ApplicationConversionService(final StringValueResolver embeddedValueResolver, final boolean unmodifiable) {
    ...
    public void addFormatterForFieldAnnotation(
        final AnnotationFormatterFactory<? extends Annotation> annotationFormatterFactory) {
        assertModifiable();
        super.addFormatterForFieldAnnotation(annotationFormatterFactory);
    }

    @Override
    public <S, T> void addConverter(final Class<S> sourceType,
    ...
    /**
     * Return {@code true} if objects of {@code sourceType} can be converted to the {@code targetType} and the converter
     * has {@code Object.class} as a supported source type.
     *
     * @param sourceType
     ...
     * @since 2.4.3
     */
    public boolean isConvertViaObjectSourceType(final TypeDescriptor sourceType, final TypeDescriptor targetType) {
        final GenericConverter converter = getConverter(sourceType, targetType);
        final Set<ConvertiblePair> pairs = converter != null ? converter.getConvertibleTypes() : null;
        if (pairs != null) {
    ...
    /**
     * Return a shared default application {@code ConversionService} instance, lazily building it once needed.
     * <p>
     * Note: This method actually returns an {@link ApplicationConversionService} instance. However, the
     * {@code ConversionService} signature has been preserved for binary compatibility.
     *
     * @return the shared {@code ApplicationConversionService} instance (never {@code null})
     */
    ...
    /**
     * Configure the given {@link FormatterRegistry} with formatters and converters appropriate for most Spring Boot
     * applications.
     *
     * @param registry
     ...
     */
    public static void configure(final FormatterRegistry registry) {
     ...
     */
    public static void addApplicationConverters(final ConverterRegistry registry) {
    ...
    private static void addApplicationConverters(final ConverterRegistry registry,
     ...
     *             if the given ConverterRegistry could not be cast to a ConversionService
     */
    public static void addDelimitedStringConverters(final ConverterRegistry registry) {
        final ConversionService service = (ConversionService) registry;
     ...
     */
    public static void addApplicationFormatters(final FormatterRegistry registry) {
    ...
    /**
     * Add {@link GenericConverter}, {@link Converter}, {@link Printer}, {@link Parser} and {@link Formatter} beans from
     ...
     * @since 2.2.0
     */
    public static void addBeans(final FormatterRegistry registry, final ListableBeanFactory beanFactory) {
        final Set<Object> beans = new LinkedHashSet<>();
